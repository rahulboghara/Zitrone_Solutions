...
<?php  else : ?>
<div id="posts-content"  class="container" >
   <h2 class="section-title"><?php esc_html_e( 'Nothing Found', 'zitronesolutions' ); ?></h2>
   <?php if ( is_home() && current_user_can( 'publish_posts' ) ) : ?>
   <p><?php printf( esc_html__( 'Ready to publish your first post?', 'zitronesolutions' ), esc_url( admin_url( 'post-new.php' ) ) ); ?></p>
   <?php elseif ( is_search() ) : ?>
   <p><?php esc_html_e( 'Sorry, but nothing matched your search terms. Please try again using different keywords.', 'zitronesolutions' ); ?></p>
   <?php get_search_form(); ?>
   <?php else : ?>
   <p><?php esc_html_e( 'It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.', 'zitronesolutions' ); ?></p>
   <?php get_search_form(); ?>
   <?php endif; ?>
</div>
<?php endif; ?>
